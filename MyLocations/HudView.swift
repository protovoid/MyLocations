...
class HudView: UIView {
  ...
  func show(animated: Bool) {
    if animated {
      ...
      // UIView.animate(withDuration: 0.3, animations: { self.alpha = 1; self.transform = CGAffineTransform.identity})
      UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: [], animations: {self.alpha = 1; self.transform = CGAffineTransform.identity}, completion: nil)
    }
  }
